...
void keyboardCallback(const std_msgs::Char::ConstPtr& msg)
{
    ...
    case 'a':
        ...
        break;
    case 'n':
        SetExposureAndGain(5000, 22.0); // Adjusted values
        current_mode_ = "Mode 1";
        break;
    case 'm':
        SetExposureAndGain(3000.0, 5.0);
...
bool isOverExposed(const cv::Mat& hist, int threshold = 255, double fraction = 0.05) {
...
void MinimizeOverexposure(bool overExposed)
{
    try
    {
        ...
        static double previousErrorRatio = 0.0;
        static double integralRatio = 0.0;

        // Define target brightness range
        // double targetBrightnessMin = 50.0;
        double targetBrightnessMax = 90.0;

        // Calculate PID error
        double error = avgBrightness - targetBrightnessMax;
        if (error<0){
            error =0;
        }
        integral += error;
        ...
        double pidAdjustmentBrightness = kp * error + ki * integral + kd * derivative;

        // Calculate overexposure ratio error
        // double targetRatio = 0.01; // Target ratio for overexposure
        double targetRatio = 0.0; // Target ratio for overexposure
        double ratioError = ratio - targetRatio;
        integralRatio += ratioError;
        double derivativeRatio = ratioError - previousErrorRatio;
        previousErrorRatio = ratioError;

        double pidAdjustmentRatio = kp * ratioError + ki * integralRatio + kd * derivativeRatio;

        // Combine brightness and ratio adjustments
        double combinedAdjustment = pidAdjustmentBrightness*0.1 + pidAdjustmentRatio*30;
        // double combinedAdjustment = pidAdjustmentRatio;


        // Apply different PID adjustments based on overexposure
        if (overExposed)
        {
            // Apply more significant adjustments if overexposure is detected
            combinedAdjustment *= 4.0;
            ROS_INFO("During Significant adjustment");
        }
        else
        {
            // Apply smaller adjustments if not overexposed
            combinedAdjustment *= 0.5;
            ROS_INFO("During Small adjustment");
        }
        
        combinedAdjustment *= 6.0;//5hz
        // Always gain first
        if(combinedAdjustment>0){

             double newGain = std::max(cam_->Gain.GetMin(), 
                                      std::min(cam_->Gain.GetMax(), gain_ - combinedAdjustment*0.1));
            SetExposureAndGain(exposure_time_, newGain);
            // Adjust gain first, then exposure time if necessary
            if (gain_ < cam_->Gain.GetMin()+1)
            {
                double newExposure = std::max(cam_->ExposureTime.GetMin(), 
                                            std::min(cam_->ExposureTime.GetMax(), exposure_time_ - combinedAdjustment));
